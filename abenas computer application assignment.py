...
"""
 Created on Fri Feb 17 12:05:42 2023
Kankam Abena 
6939121

Github account:mskankam
Github link:
    """
...
V =w*(L/2-x)
print()
print('(e) The shearforce for each step along the span is {}'.format(V))
...
b=-L
C=(L**2/6)+(2*m_AM)/w
#Using the Almighty formula the two toots are;
discriminant=b**2-4*a*c
...
#Question g
"""
Let the relatives error be r_e 
...
root_2=(-b+np.sqrt(discriminant))/2*a
print()
print('(h.2) The points at which the minimum bending moment occur sre {0} and {1}'.format(root_1,root_2))
